...
if (deployColor == "blue") {
    ...
    def applyGreenEnablePayload = "{\"data\": {\"type\":\"runs\",\"relationships\": {\"workspace\": {\"data\": {\"type\": \"workspaces\",\"id\": \"${workSpaceId}\"}}}}}"
    ...
    //println applyGreenEnable
    //applyGreenEnable = jsonSlurper.parseText(applyGreenEnable)
    //def applyGreenEnable = ["curl", "-H", "Authorization: Bearer ${token}", "-H","Content-Type: application/vnd.api+json", "-X", "POST", "-d","{\"comment\":\"Enable Green\"}", "https://app.terraform.io/api/v2/runs/${applyGreenEnable.data.id}/actions/apply"].execute().text
    //applyGreenEnable = jsonSlurper.parseText(applyGreenEnable)
    //println applyGreenEnable.data.id
    //println applyGreenEnable
    ...
    def applyPromoteGreenPayload = "{\"data\": {\"type\":\"runs\",\"relationships\": {\"workspace\": {\"data\": {\"type\": \"workspaces\",\"id\": \"${workSpaceId}\"}}}}}"
    ...
    def applyBlueEnablePayload = "{\"data\": {\"type\":\"runs\",\"relationships\": {\"workspace\": {\"data\": {\"type\": \"workspaces\",\"id\": \"${workSpaceId}\"}}}}}"
    ...
    def applyPromoteBluePayload = "{\"data\": {\"attributes\": {\"message\": \"Promote Blue\"},\"type\":\"runs\",\"relationships\": {\"workspace\": {\"data\": {\"type\": \"workspaces\",\"id\": \"${workSpaceId}\"}}}}}"
